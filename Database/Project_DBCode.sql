...
create table chickenstatus(
    ...
    chicken_status Enum('Giống','Đang phát triển','Xuất chuồng') not null,
...
create table cage(
    ...
    cage_status enum('Đang hoạt động','Ngừng hoạt động','Bảo trì') not null
);
...
-- drop trigger before_chicken_insert;
delimiter $$
...
DELIMITER $$
CREATE PROCEDURE `AddStatus`(in id int, in quantity int, in status enum('Giong','Dang Lon','Xuat Chuong'))
BEGIN
	insert into chickenstatus(chicken_id,quantity,chicken_status) values
    (id,quantity,status);
END$$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE `UpdateChickenStatus`(in id int, in quantity int, in status Enum('Giong','Dang Lon','Xuat Chuong'))
BEGIN
	update chickenstatus
    set quantity = quantity
    where chicken_id = id and chicken_status = status;
END$$
DELIMITER ;
...
begin
    ...
    if((select count(quantity) from chickenstatus where chicken_id = new.chicken_id and chicken_status = new.chicken_status) = 1)
